...
plot.region <- function(pk, pk.corr, gene, genes.in.p, p.roi, D=NULL){
  ...
  if (is.cn.avail) {
     ...
     p0 = p0 + scale_fill_manual(name="", values=c("amp"="#c994c7", "del"="#2c7fb8"), labels=c("amp"="Gain", "del"="Loss"), drop=FALSE,
  ...
  #prepare breaks and labs in Mb
   ...
   dup_del$svtype <- factor(dup_del$svtype, levels=c('DUP', 'DEL'))
    ...
    + theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(),
            ...
            legend.key.size = unit(0.6,"cm"), legend.title=element_text(size=12, face="bold"), legend.text=element_text(size=10))
    + scale_color_manual(name="", values=c('DUP'='#c994c7', 'DEL'='#2c7fb8'), guide = guide_legend(override.aes = list(size = 7)))
  ...
  p2 = p2 + theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(),
   ...
   p2 = p2 + scale_fill_manual(name="SV type", values=c('BND'='#2ca25f','INS'='#fec44f', 'INV'='#b53f4d', 'DUP'='#c994c7', 'DEL'='#2c7fb8'), 
  ...
  if (is.chip.avail) {
     ...
     #p3 = p3 + annotate("text", label="H3K27ac", x=min(x$pos), y= max.cov-0.5, size=2)
