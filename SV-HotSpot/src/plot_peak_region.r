...
plot.region <- function(pk, pk.corr, gene, genes.in.p, p.roi, D=NULL){
  ...
  inc = max(abs(x2$num.samples))*0.25
  sv.brks1 = seq(min(x2$num.samples), 0,inc)
  sv.brks2 = seq(0, max(x2$num.samples),inc)
  sv.brks = c(sv.brks1, sv.brks2)
  sv.lbls = abs(sv.brks)
...
for (j in 1:length(effected.genes)) {
	...
	myCMPlist = c(ampCMPlist, delCMPlist)
        ...
        title.size = length(levels(factor(g.exp$group))) * 3.5  
        
      	e1 <- ggplot(g.exp, aes(x=group, y=log2(gene.exp+1))) + geom_boxplot(aes(fill=group)) + theme_bw()
      	#e1 <- e1 + stat_summary(fun.data = give.n, geom = "text", size=5) 
      	e1 <- e1 + labs(x='', y='Log2(expression)') + ggtitle('Expression Profiles of SV samples with CN status')
      	e1 <- e1 + theme(axis.text.x=element_text(size=12, vjust=0.5, color="black"),
                       ...
                       plot.title = element_text(size = title.size, hjust=0.5, color="black", face="bold"),
                       legend.position="none")
      	e1 = e1 + scale_fill_manual(name="", values = c("GNPN"="gray", "GAPN"="#f03b20", "GNPA"="#b53f4d", "GAPA"="salmon", 
                                                        "GDPN"="#a6bddb", "GNPD"="#2c7fb8", "GDPD"="skyblue2")) 
      	e1 = e1 + scale_x_discrete(labels=lbls)
        e1 = e1 + geom_signif(comparisons=myCMPlist , step_increase=0.1)
      }

      ### plot the expression for SV samples
      g.exp$sample.status <- factor(g.exp$sample.status, levels=c('non-SVs', 'SVs'))
      e2 <- ggplot(g.exp, aes(x=sample.status, y=log2(gene.exp+1))) + geom_boxplot(aes(fill=sample.status)) + theme_bw() 
      e2 <- e2 + labs(x='', y='Log2(expression)') + ggtitle('Expression Profiles of SV samples')
      e2 <- e2 + theme(axis.text.x=element_text(size=12, vjust=0.5, color="black"),
                       ...
                       plot.title = element_text(size = title.size, hjust=0.5, color="black", face="bold"),
                       legend.position="none")
      e2 = e2 + scale_fill_manual(name="", values =c("non-SVs"="gray", "SVs"="orange2"))
      e2 = e2 + scale_x_discrete(labels=c(paste0("non-SVs\n(n=",nrow(g.exp[g.exp$sample.status=="non-SVs",]),")"), paste0("SVs\n(n=",nrow( g.exp[g.exp$sample.status=="SVs",]),")")))
      e2 = e2 + geom_signif(comparisons=list(c('non-SVs','SVs')))

      #### plot all 
      pdf(paste0(out.dir,'/',g,'_',pk,".pdf"), width=20, height=10, title='', useDingbats=F, onefile=FALSE)
      ...
      all.plots[[length(p.reg)+1]] = e1
      all.plots[[length(p.reg)+2]] = e2
