...
res <- res[res$p.name==pk, ]
if (nrow(res) == 0 ) { stop ('This peak was not found in the results.') } 

### create directory for plots 
dir.create(paste0(out.dir,'/', pk))
...
if (is.cn.avail) {
   ...
   if (file.exists(paste0(res.dir,'/processed_cn_data/peaks_with_cn.bed'))) {
      cat('Reading peaks copy number data...\n')
      pks.cn <- read.table(paste0(res,'/processed_cn_data/peaks_with_cn.bed'), header =T, check.names=F, sep="\t", stringsAsFactors = F)
   } else {
      stop ("Peaks copy number file doesn't exist!.\n")
   }

  ### read genes with copy number data (processed file)
  if (file.exists(paste0(res.dir,'/processed_cn_data/genes_with_cn.bed'))) {
     cat('Reading genes copy number data...\n')
     genes.cn <- read.table(paste0(res.dir,'/processed_cn_data/genes_with_cn.bed'), header =T, check.names=F, sep="\t", stringsAsFactors = F)
  } else {
...
for (j in 1:length(effected.genes)) {
	...
	myCMPlist = c(ampCMPlist, delCMPlist)
      ...
      #### plot all 
      pdf(paste0(out.dir,'/', pk, '/',g,'.pdf'), width=20, height=10, title='', useDingbats=F, onefile=FALSE)
      #png(paste0(res.dir,'/peak-gene-plots/',g,'_',pk,".png"), width=1920, height=960, units = "px", res=300 )
